...
if (isset($_POST['toggle_votacion']) && $user['position'] == 'A') {
    $query = "UPDATE settings SET votacion_ena = NOT votacion_ena";
    mysqli_query($conn, $query);
}

// Obtener el estado actual del botón de votación
$query = "SELECT votacion_ena FROM settings LIMIT 1";
$result = mysqli_query($conn, $query);
$votacion_ena = $result && mysqli_num_rows($result) > 0 ? mysqli_fetch_assoc($result)['votacion_ena'] : 0;
...
<body>
        ...
        <div class="left-section user-info">
            ...
            <?php if ($user) : ?>
                ...
                <?php if ($user['position'] == 'A') : ?>
                        <form action="enfermeria.php" method="POST">
                            <button type="submit" name="toggle_votacion" class="user-button">
                                <?php echo $votacion_ena ? 'Desactivar Registro' : 'Activar Registro'; ?>
                            </button>
                        </form>
                    <?php endif; ?>
                <form>
    <button type="button" onclick="location.href='departamentos.php'" class="user-button">
        ...
        <button class="back-button" onclick="location.href='departamentos.php'">
                <i class="fas fa-arrow-left"></i> Regresar
            </button>
            <br>
            <br>
        <h1 class="department-title">Enfermeria</h1>

            <div class="button-container">

                <div class="button-grid">
                <?php if ($user['position'] == 'A' || $votacion_ena) : ?>

                    <button onclick="location.href='enfermeria_register.php'">
                        <i class="fas fa-calendar-check icon-large"></i>
                        <br>
                        <div>Registro a eventos</div>
                    </button>
                    <?php endif; ?>
                
                <?php if ($user['position'] == 'A') : ?>

                <button onclick="location.href='enfermeria_admin.php'">
                        <i class="fas fa-calendar-alt icon-large"></i>
                        <br>
                        <div>Administrar fechas de eventos</div>
                    </button>
                    <?php endif; ?>
                    <?php if ($user['position'] == 'A') : ?>

<button onclick="location.href='enfermeriaeven.php'">
        <i class="fas fa-file-alt icon-large"></i>
        <br>
        <div>Administrar Registros</div>
    </button>
    <?php endif; ?>
                    <?php if ($user['position'] == 'A'||$user && $user['position'] == 'AB') : ?>
...
<i class="fas fa-check-circle icon-large"></i>
    ...
    <?php endif; ?>
    <button onclick="location.href='cafeteria.php'">
        <i class="fas fa-coffee icon-large"></i>
        <br>
        <div>Menú de cafeteria</div>
    </button>
                </div>
