...
if (isset($_SESSION['user_id'])) {
    ...
    $query = "SELECT firstname, lastname, position, area, plant, birthdate, username FROM users WHERE id = '$user_id'";
...
// Lista de números de reloj permitidos
$allsup = ['548', '88114', '98382'];

// Verificar propuestas pendientes en la tabla operators
if (in_array($username, $allsup)) {
    $query = "
        SELECT COUNT(*) as propuestas_pendientes 
        FROM operators o
        LEFT JOIN approvals a ON o.id = a.proposal_id AND a.status = 'approved'
        WHERE a.proposal_id IS NULL AND o.supervisor_id = '$user_id'
    ";
    $result = mysqli_query($conn, $query);
    $propuestas_pendientes = $result && mysqli_num_rows($result) > 0 ? mysqli_fetch_assoc($result)['propuestas_pendientes'] : 0;

    if ($propuestas_pendientes > 0) {
        echo "<script>alert('Tienes $propuestas_pendientes propuestas pendientes en la tabla operators.');</script>";
    }
}

// Verificar aprobaciones pendientes en temporary_approvals
$query = "
    SELECT COUNT(*) as pendientes_aprobaciones 
    FROM temporary_approvals ta
    LEFT JOIN approvals a ON ta.proposal_id = a.proposal_id AND a.status = 'approved'
    WHERE ta.approve_user_id = '$user_id' AND a.proposal_id IS NULL
";
$result = mysqli_query($conn, $query);
$pendientes_aprobaciones = $result && mysqli_num_rows($result) > 0 ? mysqli_fetch_assoc($result)['pendientes_aprobaciones'] : 0;

if ($pendientes_aprobaciones > 0) {
    echo "<script>alert('Tienes $pendientes_aprobaciones aprobaciones pendientes en la tabla temporary_approvals.');</script>";
}
        ...
        <h1 class="department-title">Departamentos de Recursos Humanos</h1>
            ...
            <div class="button-container">
                ...
                <div class="button-grid">
                    <button onclick="location.href='do.php'">
                    <i class="fas fa-sitemap icon-large"></i>
                        <br>
                        <div>Desarrollo Organizacional</div>
                    </button>
