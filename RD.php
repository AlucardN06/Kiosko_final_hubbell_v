...
// Procesar la creación de encuestas
if (isset($_POST['create_survey']) && $user['position'] == 'A') {
    $title = mysqli_real_escape_string($conn, $_POST['title']);
    $description = mysqli_real_escape_string($conn, $_POST['description']);
    $query = "INSERT INTO surveys (title, description) VALUES ('$title', '$description')";
    mysqli_query($conn, $query);
}

// Obtener todas las encuestas
$query = "SELECT * FROM surveys";
$surveys_result = mysqli_query($conn, $query);
        ...
        <div class="right-section">
            <?php if ($user['position'] == 'A') : ?>
                ...
                <form action="index.php" method="POST">
                    ...
                    <button type="submit" name="create_survey">Crear Encuesta</button>
                </form>
                <h2>Encuestas Disponibles</h2>
                <ul>
                    <?php while ($survey = mysqli_fetch_assoc($surveys_result)) : ?>
                        <li>
                            <a href="ver_encuesta.php?id=<?php echo $survey['id']; ?>">
            ...
            <?php else : ?>
                <h2>No tiene permiso para crear encuestas.</h2>
            <?php endif; ?>
        </div>
    </div>
</body>
