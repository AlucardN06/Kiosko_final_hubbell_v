...
if ($allsup=['548', '96695', '88114']&& $user['username']=='548'||'96695'||'88114' ){
    $perm = true;
    ...
    if (isset($_POST['approve_request']) || isset($_POST['reject_request'])) {
        ...
        if ($check_result && mysqli_num_rows($check_result) == 0) {
            ...
            if ($count_row['approval_count'] >= 3) {
                // Verificar que los aprobadores incluyan al supervisor, el usuario 548 y uno de los otros dos
                $supervisor_id_query = "SELECT supervisor_id FROM operators WHERE id = '$request_id'";
                $supervisor_id_result = mysqli_query($conn, $supervisor_id_query);
                $supervisor_id_row = mysqli_fetch_assoc($supervisor_id_result);
                $supervisor_id = $supervisor_id_row['supervisor_id'];
                ...
                while ($admin_row = mysqli_fetch_assoc($admin_result)) {
                    $approved_admins[] = $admin_row['admin_id'];
                }

                if (in_array('548', $approved_admins) && in_array($supervisor_id, $approved_admins) && 
                    (in_array('96695', $approved_admins) || in_array('88114', $approved_admins))) {
...
$supervisor_reloj_number = $user['supervisor_reloj_number']; // Obtenemos el supervisor_reloj_number del usuario
    ...
    if (isset($_POST['add_operator'])) {
        ...
        if ($current_supervisor_result && mysqli_num_rows($current_supervisor_result) > 0) {
            ...
            $supervisor_number =$current_supervisor_row['supervisor_reloj_number'];
        } else {
        ...
        $employee_query = "SELECT id, firstname, lastname, area, plant, colony, username, supervisor_reloj_number FROM users WHERE username = '$employee_username'";
        ...
        if ($employee_result && mysqli_num_rows($employee_result) > 0) {
            ...
            // Verificar si el supervisor actual es el supervisor asignado al empleado
            ...
            $insert_operator_query = "INSERT INTO operators (supervisor_reloj_number, employee_id, supervisor_id, supervisor_firstname, supervisor_lastname, username, firstname, lastname, area, plant, colony, proposal_date, schedule, start_time, end_time) 
                                    VALUES ('$supervisor_reloj_number', '$employee_id', '$supervisor_id', '$supervisor_firstname', '$supervisor_lastname', '$employee_username', '$employee_firstname', '$employee_lastname', '$employee_area', '$employee_plant', '$employee_colony', '$proposal_date', '$schedule', '$start_time', '$end_time')";
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="right-section">
                ...
                <?php if ($user && $user['position'] == 'AB'|| $has_temporary_approval) : ?>
            ...
            <?php if ( $perm == 1 ) : ?>
                    ...
                    <?php if ($requests_result && mysqli_num_rows($requests_result) > 0) : ?>
                        ...
                        <?php endwhile; ?>
                    <?php elseif ($user && $user['position'] == 'AB'): ?>
                ...
                <?php if ($supervisor_proposals_result && mysqli_num_rows($supervisor_proposals_result) > 0) : ?>
                    <?php if ($user && $user['position'] == 'AB'||$user && $user['position'] == 'B') : ?>
