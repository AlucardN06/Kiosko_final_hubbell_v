...
// Consultar las encuestas disponibles
if ($user['position'] == 'Administrativo') {
    $query_surveys = "SELECT id, title FROM surveys";
    $surveys_result = mysqli_query($conn, $query_surveys);
    if (!$surveys_result) {
        die("Error al obtener las encuestas: " . mysqli_error($conn));
    }
}
        ...
        <div class="right-section">
            <?php if ($user['position'] == 'Administrativo') : ?>
                ...
                <form action="crear_encuesta.php" method="POST">
                    <!-- Formulario para crear encuestas (ya definido previamente) -->
                </form>
                <h2>Encuestas Disponibles</h2>
                <ul>
                    <?php while ($survey = mysqli_fetch_assoc($surveys_result)) : ?>
                        <li>
                            <a href="editar_encuesta.php?id=<?php echo $survey['id']; ?>">
